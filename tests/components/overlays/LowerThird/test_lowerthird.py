...
class TestLowerThirdToolRegistration:
    ...
    def test_tool_execution(self):
        ...
        composition.add_lower_third = Mock()

        register_tool(mcp, project_manager)
        tool_func = mcp.tool.call_args[0][0]

        result = asyncio.run(tool_func(name="John Doe", start_time=0.0))

        assert composition.add_lower_third.called
    ...
    def test_tool_execution_error_handling(self):
        ...
        composition = Mock()
        project_manager.current_composition = composition
        composition.add_lower_third = Mock(side_effect=Exception("Test error"))
