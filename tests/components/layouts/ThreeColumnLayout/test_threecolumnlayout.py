...
class TestThreeColumnLayoutBuilderMethod:
    """Tests for ThreeColumnLayout builder method."""
    ...
    def test_add_to_composition_basic(self):
        ...
        assert builder.components[0].component_type == "ThreeColumnLayout"
    ...
    def test_add_to_composition_all_props(self):
        ...
        assert props["left"] == {"type": "left"}
        assert props["center"] == {"type": "center"}
        assert props["right"] == {"type": "right"}
        assert props["left_width"] == 30.0
        assert props["center_width"] == 40.0
        assert props["right_width"] == 30.0
...
class TestThreeColumnLayoutToolRegistration:
    """Tests for ThreeColumnLayout MCP tool registration."""
    ...
    def test_tool_execution(self):
        ...
        assert result_data["component"] == "ThreeColumnLayout"
    ...
    def test_tool_execution_error_handling(self):
        ...
        with patch.object(builder, "add_three_column_layout", side_effect=Exception("Test error")):
            result = asyncio.run(tool_func())
            result_data = json.loads(result)
            assert "error" in result_data
    ...
    def test_tool_json_parsing_error(self):
        ...
        result = asyncio.run(tool_func(left="invalid json {"))
        result_data = json.loads(result)
        assert "error" in result_data
