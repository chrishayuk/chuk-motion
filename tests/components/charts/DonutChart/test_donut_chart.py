...
class TestDonutChartToolRegistration:
    ...
    def test_tool_execution(self):
        ...
        composition.add_donut_chart = Mock()

        register_tool(mcp, project_manager)
        tool_func = mcp.tool.call_args[0][0]

        result = asyncio.run(tool_func(data='[{"label": "A", "value": 30}]', start_time=0.0))

        assert composition.add_donut_chart.called
    ...
    def test_tool_execution_error_handling(self):
        ...
        composition = Mock()
        project_manager.current_composition = composition
        composition.add_donut_chart = Mock(side_effect=Exception("Test error"))
