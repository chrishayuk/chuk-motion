...
def register_tool(mcp, project_manager):
    ...
    async def remotion_add_pie_chart(
        data: str, title: str | None = None, start_time: float = 0.0, duration: float = 4.0
    ) -> str:
        """
        ...
        Example:
            await remotion_add_pie_chart(
                data='[{"label": "Desktop", "value": 45}, {"label": "Mobile", "value": 35}]',
                title="Example Chart",
                start_time=3.0,
                duration=4.0
            )
        ...
        def _add():
            ...
            try:
                project_manager.current_composition.add_pie_chart(
                    data=data_parsed, title=title, start_time=start_time, duration=duration
                )
